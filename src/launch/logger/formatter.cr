    ...
    struct Formatter < Dexter::BaseFormatter
      ...
      private class RequestStartedFormatter < EntryFormatter
        def should_format? : Bool
          (Handler::REQUEST_START_KEYS.values.to_a - local_context.keys).empty?
        end
      ...
      private class RequestEndedFormatter < EntryFormatter
        def should_format? : Bool
          (Handler::REQUEST_END_KEYS.values.to_a - local_context.keys).empty?
        end

        def write : Nil
          add_arrow
          http_status = Helpers.colored_http_status(local_context["status"].as_i)
